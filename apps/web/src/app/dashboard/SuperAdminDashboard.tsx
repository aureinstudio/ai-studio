...
/**
 * Super admin / admin 통합 대시보드.
 * 전체 기능 노출 — 작업 통계 · 비용 · 강사·SME·운영 도구 · 학습자 도구 · 수강 과정 · 최근 작업.
 *
 * 분리 이유: 슈퍼관리자는 모든 역할 기능에 접근해야 하므로 일반 /admin 대시보드로 단순 redirect할 수 없음.
 */
type Profile = {
  id: string;
  email: string;
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-10 lg:py-24">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {profile.role === "keg_super_admin" && (
          <Link
            href="/super-admin"
            className="rounded-md border border-foreground/30 bg-foreground/5 px-4 py-2 text-sm font-medium hover:bg-foreground/10"
          >
            👑 Super Admin →
          </Link>
        )}
